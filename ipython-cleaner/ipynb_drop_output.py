...
By default this program will suppress outputs on all ipynb files! If you don't want it to suppress outputs add the line:
...
#modified from the original gist to suppress outputs by default - sorry other ipynb projects, but data projection is important!
if "git" in nb_metadata:
    if "suppress_outputs" in nb_metadata["git"] and nb_metadata["git"]["suppress_outputs"] == False:
